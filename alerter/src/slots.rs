...
use crate::blocks::{Block, BlockNumberFor, BlocksStream, Slot, Timestamp};
...
type Cache = BTreeMap<BlockNumberFor<Block>, SlotAndTimestamp>;
...
pub(crate) async fn monitor_chain_slots(
    ...
    loop {
        ...
        if let Some(reorg_data) = blocks_ext.maybe_reorg_data {
            ...
            remove_cached_blocks_until(&mut cache, common_block);
        }
...
fn remove_cached_blocks_until(cache: &mut Cache, number: BlockNumberFor<Block>) {
